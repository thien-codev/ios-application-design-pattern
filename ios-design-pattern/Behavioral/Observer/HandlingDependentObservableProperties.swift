//
//  HandlingDependentObservableProperties.swift
//  ios-design-pattern
//
//  Created by Nguyen Thien on 15/12/24.
//

import Foundation

protocol DependentObservableInvocable
{
  func invoke(_ data: Any)
}

public protocol DependentObservableDisposable
{
  func dispose()
}

public class DependentObservableEvent<T>
{
  public typealias DependentObservableEventHandler = (T) -> ()

  var eventHandlers = [DependentObservableInvocable]()

  public func raise(_ data: T)
  {
    for handler in self.eventHandlers
    {
      handler.invoke(data)
    }
  }

  public func addHandler<U: AnyObject>
    (_ target: U, _ handler: @escaping (U) -> DependentObservableEventHandler) -> DependentObservableDisposable
  {
    let subscription = DependentObservableSubscription(target: target, handler: handler, event: self)
    eventHandlers.append(subscription)
    return subscription
  }
}

class DependentObservableSubscription<T: AnyObject, U> : DependentObservableInvocable, DependentObservableDisposable
{
  weak var target: T?
  let handler: (T) -> (U) -> ()
  let event: DependentObservableEvent<U>

  init(target: T?, handler: @escaping (T) -> (U) -> (), event: DependentObservableEvent<U>)
  {
    self.target = target
    self.handler = handler
    self.event = event
  }

  func invoke(_ data: Any) {
    if let t = target {
      handler(t)(data as! U)
    }
  }

  func dispose()
  {
    event.eventHandlers = event.eventHandlers.filter { $0 as AnyObject? !== self }
  }
}

class DependentObservablePerson
{
  private var oldCanVote = false
  var age: Int = 0
  {
    willSet(newValue)
    {
      print("About to set age to \(newValue)")
      oldCanVote = canVote // somewhat silly
    }
    didSet // cannot have a set specifier!
    {
      print("We just changed age from \(oldValue) to \(age)")

      if age != oldValue // really did change
      {
        propertyChanged.raise(("age", age))
      }

      // did we affect canVote?
      if canVote != oldCanVote
      {
        propertyChanged.raise(("canVote", canVote))
      }
    }
  }

  var canVote: Bool // dependent property
  {
    get {
      return age >= 16
      // now do we get notifications here?
    }
  }

  // C# style property changed notification
  let propertyChanged = DependentObservableEvent<(String, Any)>()
}

// pass a bool by reference without the 'inout' bother
final class RefBool
{
  var value: Bool
  init(_ value: Bool)
  {
    self.value = value
  }
}

class Person2
{
  private var _age: Int = 0
  var age: Int
  {
    get {
      return _age
    }

    set(value) {
      if _age == value { return } // no change happened

      // cache all dependent properties
      let oldCanVote = canVote

      // does anyone want to prevent this property being set?
      var cancelSet = RefBool(false)
      propertyChanging.raise(("age", value, cancelSet))
      if (cancelSet.value)
      {
        return // someone canceled
      }

      // assign and notify
      _age = value
      propertyChanged.raise(("age", _age))

      // check all dependants that changed
      if oldCanVote != canVote
      {
        propertyChanged.raise(("canVote", canVote))
      }
    }
  }

  var canVote: Bool
  {
    return _age >= 16
  }

  let propertyChanged = Event<(String, Any)>()

  // feel free to also implement
  let propertyChanging = Event<(String, Any, RefBool)>()
  // for handling value that's about to change
}

class DependentObservableDemo
{
  init()
  {
    let p = DependentObservablePerson()
    p.propertyChanged.addHandler(self, DependentObservableDemo.propChanged)
    p.age = 20
    p.age = 22 // voting status shouldn't change

    print("===")

    let p2 = Person2()
      p2.propertyChanged.addHandler(target: self, handler: DependentObservableDemo.propChanged)
      p2.propertyChanging.addHandler(target: self, handler: DependentObservableDemo.propChanging)
    p2.age = 20
    p2.age = 22

    p2.age = 12 // disallowed
  }

  func propChanging(args: (String, Any, RefBool))
  {
    if args.0 == "age" && (args.1 as! Int) < 14
    {
      print("Cannot allow to set the age lower than 14")
      args.2.value = true // cancel this
    }
  }

  func propChanged(args: (String, Any))
  {
    if args.0 == "age"
    {
      print("Person's age has been changed to \(args.1)")
    }
    else if args.0 == "canVote"
    {
      print("Voting status has been changed to \(args.1)")
    }
  }
}
